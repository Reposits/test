...
local nav = main:Navigation()

local homeTab = main:CreateTab({
	name = "Home",
	icon = "rbxassetid://12755723208"
})

local silentTab = main:CreateTab({
	name = "Silent",
	icon = "rbxassetid://6523858394"
})

local blatantTab = main:CreateTab({
	name = "Blatant",
...
local warning = homeTab:Warning()

warning:SetText("John")

local fovSlider = silentTab:Slider({
	title = "Field of View",
	min = 1,
	max = 120,
	default = 70,
...
local info = settingsTab:Info({
	title = " "
})

local textBox = settingsTab:Textbox({
	title = "Set custom username",
	callback = function(value)
		print(value)
	end,
})

local hideUser = settingsTab:Toggle({
	title = "Allow custom usernames",
	callback = function(value) 
		local hover = false
		local toggle = false
		
		plr.PlayerGui.MergeLib.Main.cont.Settings.Textbox.Box:GetPropertyChangedSignal("Text"):Connect(function()
			if value then
				Library:tween(nav["59"], {TextTransparency = 1})
				info:SetText("Username successfully changed!")
				wait(0.2)
				nav:SetText(textBox:GetValue())
				Library:tween(nav["59"], {TextTransparency = 0})
				print(value)
			end
		end)
		if not value then
			Library:tween(nav["59"], {TextTransparency = 1})
			info:SetText("Username successfully reset!")
			wait(0.2)
			nav:SetText(plr.Name)
			Library:tween(nav["59"], {TextTransparency = 0})
		end
		...
		plr.PlayerGui.MergeLib.Main.cont.Settings.ToggleInactive.MouseEnter:Connect(function()
			hover = true
		end)
		
		plr.PlayerGui.MergeLib.Main.cont.Settings.ToggleInactive.MouseLeave:Connect(function()
			hover = false
		end)
...
local notif = main:CreateNotification({
	title = "Notification",
	header = "A mod has joined",
	description = "Goodbye.",
})